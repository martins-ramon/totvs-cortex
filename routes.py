...
def change_password():
    ...
    try:
        user = db.session.execute(text("SELECT password_hash FROM users WHERE id = :id"), {"id": session['user_id']}).fetchone()
        ...
        db.session.execute(text("UPDATE users SET password_hash = :new_hash WHERE id = :id"), {"new_hash": new_password_hash, "id": session['user_id']})
        db.session.commit()
        return jsonify({"success": True})
    except Exception as e:
        db.session.rollback()
...
def profile_photo():
    ...
    try:
        if request.method == 'POST':
            ...
            db.session.execute(text("UPDATE users SET profile_photo = :photo WHERE id = :uid"), {"photo": photo_bytes, "uid": session['user_id']})
            db.session.commit()
            return jsonify({"success": True})
        else:
            row = db.session.execute(text("SELECT profile_photo FROM users WHERE id = :uid"), {"uid": session['user_id']}).fetchone()
            if row and row[0]:
...
def user_photo(user_id):
    ...
    try:
        row = db.session.execute(text("SELECT profile_photo FROM users WHERE id = :uid"), {"uid": user_id}).fetchone()
        if row and row[0]:
            photo_base64 = base64.b64encode(row[0]).decode('utf-8')
            return jsonify({"photo": f"data:image/jpeg;base64,{photo_base64}"})
        return jsonify({"photo": None})
...
def user_insights(user_id):
    ...
    try:
        ...
        last_feedback_res = db.session.execute(
        ...
        cached_insight = db.session.execute(
        ...
        all_feedbacks_res = db.session.execute(
...
def handle_single_meeting(meeting_id):
    ...
    try:
        if request.method == 'GET':
            ...
            """)
            result = db.session.execute(query, {"mid": meeting_id, "uid": user_id}).fetchone()
            if result: 
            ...
            db.session.execute(text("DELETE FROM meeting_chunks WHERE meeting_id = :mid"), {"mid": meeting_id})
            ...
            db.session.execute(text("DELETE FROM meetings WHERE id = :mid AND user_id = :uid"), {"mid": meeting_id, "uid": session['user_id']})
            db.session.commit()
            return jsonify({"success": True})
    except Exception as e:
        db.session.rollback()
...
def share_meeting(meeting_id):
    ...
    user_id = session['user_id']
    try:
        owner_check = db.session.execute(text("SELECT u.name, m.summary FROM meetings m JOIN users u ON m.user_id = u.id WHERE m.id = :mid AND m.user_id = :uid"), {"mid": meeting_id, "uid": user_id}).fetchone()
        ...
        if request.method == 'POST':
            ...
            db.session.execute(text("DELETE FROM meeting_access WHERE meeting_id = :mid"), {"mid": meeting_id})
            ...
            result = db.session.execute(text("SELECT user_id FROM meeting_access WHERE meeting_id = :mid"), {"mid": meeting_id})
...
def get_notifications():
    ...
    try:
        ...
        unread_count_res = db.session.execute(
...
def poll_notifications():
    ...
    try:
        ...
        new_notifs_res = db.session.execute(
            text("""
                SELECT id, title, message, link, is_read, created_at, actor_id
                FROM notifications
...
def create_notification():
    ...
    sql = text("""
        INSERT INTO notifications (user_id, actor_id, title, message, link, is_read)
        VALUES (:user_id, :actor_id, :title, :message, :link, :is_read)
