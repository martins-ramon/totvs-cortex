...
- `cortex/views/` — blueprints (all under `/api`): `auth` (local login + Google OAuth), `people` (people CRUD under `/api/people`), `sessions` (core 1:1 model: `/api/oneonones`, AI extraction, commitments, prep/agenda), `checkpoints` (Feedz-style checkpoints), `cards` (async batch flow generate-start/person/generate-finish over `card_jobs`)
